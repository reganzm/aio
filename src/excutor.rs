...
pub struct Executor {
    ...
    // mark the Executor is not send and sync
        ...
        EX.set(self, || {
            ...
            loop {
                ...
                while let Some(t) = self.local_queue.pop() {
                    ...
                    let w: Waker = waker(t.clone());
                    let mut context = Context::from_waker(&w);
                    let _t = Pin::new(future).as_mut().poll(&mut context);
                }
...
impl Helper {
    ...
    unsafe fn wake_by_ref(ptr: *const ()) {
        let rc = mem::ManuallyDrop::new(unsafe { Rc::from_raw(ptr as *const Task) });
        rc.wake_by_ref_();
    }
...
unsafe fn increase_refcount(data: *const ()) {
    ...
    let rc =
        mem::ManuallyDrop::new(unsafe { Rc::clone(&Rc::<Task>::from_raw(data as *const Task)) });
